...
Muchachos entonces el menu se hace en la aplicacion donde integramos todos los tad que vamos hacer

Los TADS:
TAD SIMPLE DE CHOFER
    (CREAR, CARGAR, VER, MODIFICAR, ASIGNAR O COPIAR)

TAD COMPUESTO TIPO LISTA DE CHOFERES
    (CREAR, AGREGAR, ELIMINAR, RECUPERAR, TAMANIO)
    

    (CREAR, ENCOLAR, DESENCOLAR, TAMANIO, ESVACIA)

'''

def CrearListaChofer():
    ListaChofer = []
    return ListaChofer

def AgregarChofer(ListaChofer, nuevo):
    ListaChofer.append(nuevo)

def EliminarChofer(ListaChofer, chofer):
    ListaChofer.remove(chofer)
#pero en este nos piden que sea por legajo... entonces habria que hacer alguna modificacion desde la aplicacion.??

def RecuperarChofer(ListaChofer, i):
    return ListaChofer[i-1]

def TamanioListaChofer(ListaChofer):
    return len(ListaChofer)
